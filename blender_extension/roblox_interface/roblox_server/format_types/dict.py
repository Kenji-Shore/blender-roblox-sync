...
def read(self, format_data, **masks):
	dict_len, = unpack_from(*self.__read_buffer(format_size))
	dict_value = {}
	for _ in range(dict_len):
		k, = self.__parse(format_data["index"], **masks)
		v = self.__parse(format_data["value"], **masks)
...
def write(self, args, args_count, format_data, **masks):
	...
	self.__write_buffer(pack(dict_len), format_size)
	if dict_len > 0:
		test_value = next(iter(dict_items))[1]
		values_type = type(test_value)
		if (values_type is list) or (values_type is tuple):
			for k, v in dict_items:
				self.__parse((k,), 0, format_data["index"], **masks)
				self.__parse(v, 0, format_data["value"], **masks)
		else:
			for k, v in dict_items:
				self.__parse((k,), 0, format_data["index"], **masks)
				self.__parse((v,), 0, format_data["value"], **masks)
